...
public class SeekerDupe : MonoBehaviour
{
    //fields
    ...
    void Update()
    {
        ...
        if (!hasObject)
        {
            ...
            if (FindObjectWithOverlap() && !IsPlayerPumpkin())
            {
    ...
    bool IsPlayerPumpkin()
    {
        if (objectToRetrieve.transform.parent.CompareTag("Player"))
        {
            return true;
        }
